...
To overcome these fundamental limitations, we propose \textbf{HEDO-HVSC}, a unified multimodal architecture built on two mathematically grounded innovations:
\begin{enumerate}
...
An input image $\mathbf{I} \in \mathbb{R}^{3 \times 224 \times 224}$ and paired caption text $\mathbf{T}$ are first passed through frozen pretrained feature extractors:
\begin{align}
...
\begin{equation}
\begin{aligned}
    D_{\text{KL}}(p_I \parallel p_T) = \frac{1}{2} \sum_{d=1}^{z_{\text{dim}}} \Bigg[ & \log \frac{\sigma_{T,d}^2}{\sigma_{I,d}^2} \\
    & + \frac{\sigma_{I,d}^2 + (\mu_{I,d} - \mu_{T,d})^2}{\sigma_{T,d}^2} - 1 \Bigg]
\end{aligned}
\end{equation}
