...
\thanks{Manuscript prepared for submission. Code, audited logs, and configuration artifacts are publicly available at \protect\url{https://github.com/abhinavsai2006/PH_SSD}. (Email: \protect\url{abhinavsai2006@gmail.com})}%
...
\end{equation}
Numerical stability across discrete integration steps is subsequently monitored empirically through finite-value assertions.
...
Table~\ref{tab:configurations} defines the controlled 8-configuration ablation benchmark. While these eight configurations evaluate critical individual and joint additions (e.g., Mamba-2 + HEDO vs. baseline), they represent a structured subset of the $2^4$ architectural space rather than an exhaustive full factorial matrix; consequently, they evaluate selected combinations rather than independently isolating every factor in complete isolation (such as Mamba-2 without HEDO, or MSA without Mamba-2/HEDO). Full V2 activates all four mechanisms simultaneously and serves as a compositional stress test rather than an assumption that all components are mutually compatible.
...
The author thanks the open-source community for maintaining the \texttt{mamba\_ssm} and \texttt{transformers} libraries.
