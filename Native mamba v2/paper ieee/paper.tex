...
\author{
\IEEEauthorblockN{Madapati Naga Durga Abhinav Sai}
\IEEEauthorblockA{
...
522237, India\\
{\footnotesize abhinav.24bca7148@vitapstudent.ac.in}
}
\and
\IEEEauthorblockN{T V R M Siva Shanmukh}
\IEEEauthorblockA{
\textit{School of Computer Science}\\
\textit{and Engineering}\\
\textit{VIT-AP University}\\
Amaravati, Andhra Pradesh\\
522237, India\\
{\footnotesize shanmukh.24bce7269@vitapstudent.ac.in}
}
\and
\IEEEauthorblockN{Rajesh Duvvuru}
\IEEEauthorblockA{
\textit{School of Computer Science}\\
\textit{and Engineering}\\
\textit{VIT-AP University}\\
Amaravati, Andhra Pradesh\\
522237, India\\
{\footnotesize rajesh.duvvuru@vitap.ac.in}
}
}
...
\begin{abstract}
Structured State Space Models (SSMs) and their duality with structured attention---principally embodied in Mamba-2 via State Space Duality (SSD)---offer linear time complexity $O(L)$ with respect to sequence length, presenting an appealing alternative to quadratic self-attention in cross-modal vision-language alignment. In this paper, we conduct a rigorous, pre-specified empirical audit investigating whether continuous-time dynamical system adaptations and variational latent coupling enhance or compromise cross-modal representations built upon frozen vision (ViT-B/16) and language (RoBERTa-base) encoders. We formalize and evaluate three mechanisms: (1) Structure-Aware Hamiltonian-Inspired Dissipative Coordinate-Momentum Adapter (HEDO), incorporating exact skew-symmetric learned interactions $\mathbf{J} = (\mathbf{A} - \mathbf{A}^T)/\sqrt{d}$ and positive diagonal damping $\mathbf{R}$; (2) Adaptive Variational State Coupling (AVSC), which introduces confidence-gated symmetric Kullback-Leibler (KL) divergence across intermediate chunk-boundary states; and (3) Multi-Scale State Aggregation (MSA), fusing global mean, boundary, and terminal recurrent states. Across an audited 24-run, 8-configuration ablation benchmark (three random seeds per configuration, $n=3$ paired runs) on Flickr8k, paired-difference analysis using two-tailed 95\% Student-$t$ confidence intervals indicates: (i) Mamba-2 equipped with HEDO shows no statistically significant difference from the standard linear alignment baseline under the present three-seed evaluation ($37.31 \pm 0.68\%$ vs. $37.16 \pm 0.57\%$ Mean Recall, paired $\Delta = +0.15$ percentage points [pp], 95\% CI $[-1.42, +1.73]$ pp, CI includes zero) while processing $7,054$ pairs/s at the head with $1,086$ MB peak VRAM; (ii) Multi-scale recurrent aggregation consistently degrades retrieval performance ($\Delta \approx -10.27$ to $-10.75$ pp, with 95\% CIs strictly negative), revealing that causal boundary states introduce severe temporal asymmetry that corrupts isotropic pooling; and (iii) Variational coupling is associated with severe alignment degradation ($\Delta \approx -30.52$ to $-32.11$ pp), with retrieval falling to approximately $5.0\%-6.6\%$ Mean Recall, consistent with a collapse-like representation failure under the evaluated formulation. Geometric and gradient analyses indicate that intermediate cross-modal symmetric KL penalties introduce competing distribution-matching constraints that conflict with hyperspherical multi-positive InfoNCE contrastive repulsion, providing a plausible mechanistic interpretation of this failure. Our findings deliver vital negative results and actionable design principles for integrating state space duality into multimodal systems.
\end{abstract}
...
In the existing literature, continuous-time and variational inductive biases are frequently evaluated under isolated, non-audited configurations lacking multi-seed paired-difference analysis or systematic ablation isolation of individual components. Crucially, the interaction between sequential SSM recurrent dynamics and hyperspherical contrastive alignment manifolds has not been systematically investigated. Without rigorous ablation controls, it is impossible to determine whether reported performance variations stem from the underlying state-space sequence model, continuous-time dynamical adapters, or stochastic optimization noise.

\subsection{Research Questions and Hypotheses}
To resolve this gap, we formalize four pre-specified research questions:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Controlled Empirical Audit}: We construct a controlled 8-configuration ablation benchmark evaluating selected individual and joint architectural configurations from the $2^4$ design space upon strictly frozen ViT-B/16 and RoBERTa-base backbones.
    \item \textbf{Paired Multi-Seed Statistical Evaluation}: Across 24 audited runs (3 independent random seeds per configuration, $n=3$ paired runs), we perform paired-difference analysis using two-tailed 95\% Student-$t$ confidence intervals, finding that no statistically significant difference from the baseline was detected under the three-seed evaluation ($+0.15$ pp, 95\% CI $[-1.42, +1.73]$ pp).
    \item \textbf{Mechanistic Analysis}: We provide geometric and architectural analyses hypothesizing how causal boundary states in MSA introduce temporal asymmetry that degrades isotropic pooling ($\approx -10.5$ pp), and interpret how intermediate cross-modal symmetric KL penalties in AVSC impose conflicting distribution-matching constraints against hyperspherical InfoNCE repulsion, consistent with the observed representation collapse ($\approx -31$ pp).
\end{enumerate}
...
In representation learning, variational information bottlenecks classically regularize latent spaces by penalizing the Kullback-Leibler (KL) divergence between approximate posteriors and a static uninformative prior, typically $\mathcal{N}(\mathbf{0}, \mathbf{I})$~\cite{kingma2014auto,alemi2017deep}. In cross-modal settings, variational extensions instead couple modalities by penalizing symmetric divergence directly between vision and language posteriors without a static Gaussian anchor. However, the interaction between cross-modal posterior distribution-matching penalties and hyperspherical contrastive manifolds remains poorly understood, frequently inducing optimization instability when intermediate distributional constraints conflict with contrastive objective gradients.
...
We formalize four testable hypotheses evaluated via paired-difference confidence interval analysis:
\begin{itemize}
    ...
    \item \textbf{Hypothesis 4 ($H_4$)}: State-space sequence processing introduces measurable latency and memory overhead relative to linear projection while maintaining practical throughput.
\end{itemize}
...
\subsection{Linear Projection and Baseline Architecture}
...
\end{equation}
where $\mathbf{W}_v, \mathbf{W}_t \in \mathbb{R}^{768 \times 128}$ are trainable linear projection weight matrices initialized via Xavier uniform initialization. This linear projection reduces the channel dimensionality from 768 to 128, establishing the common working dimension for subsequent adapters and sequence models. In the reference \texttt{baseline} model, these projected tokens bypass all sequence blocks and adapters, proceeding directly to masked global mean pooling and InfoNCE alignment. This establishes the canonical minimal baseline for cross-modal alignment from frozen encoders.
...
We employ a bounded explicit Euler step with $\Delta t = 0.05 + 0.10 \cdot \sigma(\tau_{\Delta t}) \in [0.05, 0.15]$, where $\tau_{\Delta t} \in \mathbb{R}$ is a separate learnable scalar parameter distinct from the InfoNCE log-temperature $\tau$:
\begin{equation}
\label{eq:euler_step}
\mathbf{q}_{\mathrm{new}} = \mathbf{q} + \Delta t \cdot \dot{\mathbf{q}}, \quad \mathbf{p}_{\mathrm{new}} = \mathbf{p} + \Delta t \cdot \dot{\mathbf{p}}.
\end{equation}
Importantly, while Proposition 1 establishes monotonic energy decay ($\frac{d}{dt}\mathcal{H} \le 0$) for the continuous-time dynamical system \eqref{eq:hedo_ode}, discrete Euler discretization introduces local truncation errors of order $O(\Delta t^2)$ and does not mathematically guarantee strict monotonic dissipation at each discrete step. Instead, the combination of small step sizes $\Delta t \le 0.15$ and strictly positive damping $\mathbf{R} \ge 0.05\mathbf{I}$ serves as a stabilizing inductive bias, and numerical stability across discrete integration steps is monitored empirically through runtime finite-tensor assertions.
...
To evaluate selected individual and joint effects of HEDO, Mamba-2, MSA, and AVSC, we construct a controlled 8-configuration ablation benchmark, summarized in Table~\ref{tab:configurations}. Because a full factorial exploration over four binary mechanisms would require $2^4 = 16$ configurations, our benchmark evaluates a targeted, structured subset of this space.
...
\texttt{mamba2\_hedo} & $\checkmark$ & $\checkmark$ & $\times$ & $\times$ \\
\texttt{mamba2\_msa} & $\times$ & $\checkmark$ & $\checkmark$ & $\times$ \\
\texttt{hedo\_msa} & $\checkmark$ & $\times$ & $\checkmark$ & $\times$ \\
\texttt{avsc\_msa} & $\times$ & $\times$ & $\checkmark$ & $\checkmark$ \\
\texttt{mamba2\_avsc} & $\times$ & $\checkmark$ & $\times$ & $\checkmark$ \\
\texttt{full\_model} (All) & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\texttt{hedo\_avsc} & $\checkmark$ & $\times$ & $\times$ & $\checkmark$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:configurations} defines the controlled 8-configuration ablation benchmark. We intentionally designate the linear-projection model as the reference \texttt{baseline}: because the underlying ViT-B/16 and RoBERTa-base representations are frozen, direct linear projection followed by mean pooling constitutes the simplest and most parameter-efficient alignment head. Comparing against this linear baseline directly isolates whether introducing sequential state-space modeling (Mamba-2 SSD) and continuous dynamical adapters (HEDO) adds measurable empirical value over direct linear dimensionality reduction. While these eight configurations evaluate critical individual and joint additions (e.g., Mamba-2 + HEDO vs. baseline), they represent a structured subset of the $2^4$ architectural space rather than an exhaustive full factorial matrix; consequently, they evaluate selected combinations rather than independently isolating every factor in complete isolation (such as Mamba-2 without HEDO, or MSA without Mamba-2/HEDO). The full model activates all four mechanisms simultaneously and serves as a compositional stress test rather than an assumption that all components are mutually compatible.
...
\end{equation}
where $t_{0.025, 2} = 4.303$. An architectural comparison relative to the baseline is classified as:
\begin{itemize}
    \item \textbf{SUPPORTED (IMPROVEMENT)}: $\bar{\Delta}_c > 0$ and $\mathrm{CI}_{95\%}$ strictly excludes zero ($\mathrm{CI}_{\mathrm{low}} > 0$).
    ...
    \item \textbf{CONTRADICTED (DEGRADATION)}: $\bar{\Delta}_c < 0$ and $\mathrm{CI}_{95\%}$ strictly excludes zero ($\mathrm{CI}_{\mathrm{high}} < 0$).
\end{itemize}
Because the analysis was designed as a predefined architectural audit with seven baseline comparisons, we report unadjusted two-sided 95\% Student-$t$ confidence intervals; multiplicity-adjusted inference is left for future expanded benchmarks.
...
\ENSURE Paired deltas $\bar{\Delta}_c$, 95\% confidence intervals $\mathrm{CI}_{95\%}$, pre-specified verdicts
\FOR{each configuration $c \in \mathcal{C}$}
    ...
    \ENDIF
\ENDFOR
\STATE Generate tabular summaries (Tables~\ref{tab:ablation_results}, \ref{tab:paired_deltas}, \ref{tab:efficiency_results}) and statistical figures (Figs.~\ref{fig:ablation_recall}--\ref{fig:seed_stability_metrics})
...
The empirical retrieval metrics across all 8 configurations are compiled in Table~\ref{tab:ablation_results} and illustrated across recall thresholds in Fig.~\ref{fig:ablation_recall}. 
...
\textbf{Full Model (All)} & $1.47 \pm 0.31$ & $5.97 \pm 0.47$ & $10.73 \pm 0.68$ & $1.25 \pm 0.29$ & $5.60 \pm 0.69$ & $10.69 \pm 1.09$ & $5.95 \pm 0.42$ & $94.33 \pm 6.66$ & $69.67 \pm 3.79$ & Contradicted \\
...
Configurations incorporating Multi-Scale State Aggregation (\texttt{mamba2\_msa} and \texttt{hedo\_msa}) exhibit a sharp and consistent drop in retrieval capability. \texttt{mamba2\_msa} attains a Mean Recall of $26.89 \pm 0.68\%$, representing a severe paired delta of $-10.2711$ pp (95\% CI: $[-13.1018, -7.4404]$ pp). Similarly, \texttt{hedo\_msa} achieves $26.41 \pm 0.49\%$ ($\Delta = -10.7489$ pp, 95\% CI: $[-12.3816, -9.1161]$ pp). Both results strictly exclude zero and contradict Hypothesis 2 ($H_2$). Instead of enriching sequence representations, multi-scale chunk boundary and terminal token states substantially degrade retrieval quality.

\subsection{AVSC Ablation Analysis}
The most striking empirical outcome is that Adaptive Variational State Coupling is associated with severe alignment degradation. Across all four configurations containing AVSC (\texttt{avsc\_msa}, \texttt{mamba2\_avsc}, \texttt{full\_model}, and \texttt{hedo\_avsc}), Mean Recall drops precipitously to $5.04\%-6.64\%$, consistent with dimensional collapse under the evaluated formulation. 
...
\begin{itemize}
    ...
    \item \textbf{Full Model (All)}: $\Delta = -31.21$ pp (95\% CI: $[-32.60, -29.81]$ pp)
    \item \textbf{HEDO + AVSC}: $\Delta = -32.11$ pp (95\% CI: $[-35.61, -28.62]$ pp)
\end{itemize}
The full model (All), which simultaneously combines HEDO, Mamba-2, MSA, and AVSC, attains $5.95 \pm 0.42\%$ Mean Recall, indicating that the variational degradation dominates and nullifies any beneficial inductive biases. Correspondingly, Median Rank (MedR), which is $7.67$ (i2t) and $9.00$ (t2i) for Baseline and Mamba-2 + HEDO, blows up to $81.67-103.17$ (i2t) and $61.33-79.00$ (t2i) in AVSC configurations. These outcomes contradict Hypothesis 3 ($H_3$).
...
\textbf{Full Model (All) vs. Baseline} & $\{42: -31.55, 43: -30.56, 44: -31.51\}$ & $\mathbf{-31.2067}$ & $0.5604$ & $[-32.5987, -29.8146]$ & True & \textbf{CONTRADICTED (-31.21 pp)} \\
...
\textbf{Full Model (All)} & $763,931$ & $211,386,627$ & $1,089.33$ & $463.06 \pm 1.49$ & $7.07 \pm 0.45$ & $10.68 \pm 0.39$ & $5,671.25 \pm 375.35$ & $3,749.01 \pm 139.28$ \\
...
\caption{Efficiency vs. accuracy tradeoffs across all 8 configurations. (a) Head throughput (pairs/s) vs. Mean Recall (\%). (b) End-to-end throughput (pairs/s) vs. Peak Training VRAM (MB). Bubble area is proportional to the trainable parameter count ($465\mathrm{k}$ to $764\mathrm{k}$). Mamba-2 + HEDO lies on the observed accuracy-throughput Pareto frontier among the eight evaluated configurations, retaining baseline-level retrieval accuracy at high throughput and low memory.}
\label{fig:efficiency_tradeoff}
\end{figure*}

Mamba-2 + HEDO requires $597,787$ trainable parameters, adding $132,610$ trainable parameters (+28.5\% relative to the baseline trainable parameter budget of $465,177$), while increasing the total parameter count by only 0.063\% because the 210.6M backbone parameters remain frozen. Relative to the baseline linear projection, peak training VRAM increases from $1,042.31$ MB to $1,086.15$ MB ($+43.84$ MB, a 4.2\% increase), head latency increases from $4.19 \pm 0.13$ ms to $5.67 \pm 0.13$ ms (+35.3\%), and end-to-end latency increases from $7.64 \pm 0.23$ ms to $8.39 \pm 0.08$ ms (+9.8\%). Despite these moderate overheads, head throughput remains high at $7,054.12 \pm 161.35$ pairs/s ($4,765.93 \pm 45.65$ pairs/s end-to-end), supporting Hypothesis 4 ($H_4$) by confirming practical computational throughput. As shown in Fig.~\ref{fig:efficiency_tradeoff}, Mamba-2 + HEDO lies on the observed accuracy-throughput Pareto frontier among the eight evaluated configurations.

\subsection{Multi-Seed Stability}
Fig.~\ref{fig:seed_stability_metrics} illustrates the stability across seeds 42, 43, and 44. The standard deviation in Mean Recall across seeds is small ($\le 0.68\%$ for the top configurations), indicating consistency across the evaluated seeds, although larger seed ensembles are required to quantify sensitivity to random initialization more precisely.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.96\textwidth]{fig_seed_stability_metrics.pdf}
\caption{Multi-seed run stability and retrieval rank distributions. (a) Mean Recall across seeds 42, 43, and 44, showing consistent performance across the three evaluated random seeds. (b) Log-scale Median Retrieval Rank (MedR) for image-to-text and text-to-image matching; lower values denote superior retrieval ranking precision.}
...
Unlike AVSC and MSA, HEDO maintains performance with no statistically significant difference detected from the baseline under the three-seed evaluation. This stability is governed by three structural properties formalized in Section~\ref{sec:architecture}-D:
\begin{enumerate}
    ...
    \item \textbf{Positive Diagonal Damping}: The damping matrix $\mathbf{R} = \mathrm{diag}(0.05 + 0.95\sigma(\boldsymbol{\gamma}))$ via \eqref{eq:damping_matrix} guarantees that energy is dissipated monotonically in the continuous-time formulation ($\dot{\mathcal{H}} \le 0$ by Proposition 1). Under discrete Euler steps, this continuous inductive bias combined with bounded step sizes empirically prevents activation blow-up or exploding gradients.
...
We hypothesize that the degradation arises from the interaction between recurrent positional asymmetry and the selected pooling scheme:
\begin{enumerate}
    \item \textbf{Recurrent Positional Asymmetry}: In Mamba-2, state transitions are computed sequentially:
    \begin{equation}
    \label{eq:mamba_recurrence}
    \mathbf{h}_t = \mathbf{A}_t \mathbf{h}_{t-1} + \mathbf{B}_t \mathbf{x}_t.
    \end{equation}
    The terminal valid state $\mathbf{y}_{\mathrm{last}} = \mathbf{Y}_{B, L_{\mathrm{valid}}-1}$ reflects the cumulative recency bias of the sequential recurrence. In language, it corresponds to the final valid token, whose semantic or positional role may differ substantially across captions. In vision, the final token corresponds to the bottom-right image patch ($14 \times 14$ grid). Concatenating $\mathbf{y}_{\mathrm{last}}$ introduces severe positional asymmetry into the multimodal embedding.
    \item \textbf{Chunk Boundary Arbitrariness}: Chunk-boundary states $\mathbf{Y}_{\mathrm{boundary}}$ capture the internal recurrent state at arbitrary 16-token intervals. Because visual entities and linguistic phrases do not align with uniform 16-token boundaries, these states represent arbitrary recurrent snapshots rather than coherent semantic constituents.
    \item \textbf{Order-Agnostic Mean Pooling}: Conversely, uniform masked mean pooling $\bar{\mathbf{y}}_{\mathrm{global}} = \frac{\sum_{i=1}^L m_i \mathbf{Y}_i}{\sum_{i=1}^L m_i}$ computes an unweighted barycenter over all contextualized token representations, providing an order-agnostic aggregation of the contextualized sequence states suitable for contrastive alignment.
\end{enumerate}

\subsection{Mechanistic Interpretation of AVSC Degradation}
\label{sec:avsc_mechanisms}
The severe retrieval degradation observed across all AVSC configurations ($\Delta \approx -30.5$ to $-32.1$ pp, Mean Recall $\approx 5.0\%-6.6\%$, MedR $\ge 60$) demands careful mechanistic examination. Rather than a formal mathematical theorem of collapse, we provide a conservative mechanistic interpretation based on the competing geometric constraints of the objective functions.

\subsubsection{Competing Geometric Pressures}
In the evaluated architecture, two distinct objectives operate at different stages of representation processing:
\begin{enumerate}
    \item \textbf{Hyperspherical Contrastive Repulsion}: The multi-positive InfoNCE loss operates on final $\ell_2$-normalized embeddings $\mathbf{z}_v, \mathbf{z}_t \in \mathbb{S}^{d-1}$. As established in contrastive representation theory~\cite{wang2020understanding,chen2020simple}, InfoNCE balances positive attraction with negative repulsion. The repulsive gradient across negative pairs,
    \begin{equation}
    ...
    \end{equation}
    pushes non-matching embeddings apart, promoting uniform dispersion and high effective rank across the unit hypersphere $\mathbb{S}^{d-1}$.
    \item \textbf{Intermediate Distribution Matching}: Concurrently, the AVSC objective penalizes the symmetric Kullback-Leibler divergence $\mathcal{D}_{\mathrm{KL}}^{\mathrm{sym}}(q_v, q_t)$ between diagonal Gaussian posteriors $q_v = \mathcal{N}(\boldsymbol{\mu}_v, \boldsymbol{\Sigma}_v)$ and $q_t = \mathcal{N}(\boldsymbol{\mu}_t, \boldsymbol{\Sigma}_t)$ parameterized by linear heads on intermediate representations $\mathbf{h}_v, \mathbf{h}_t \in \mathbb{R}^d$ via \eqref{eq:avsc_params_v}--\eqref{eq:avsc_params_t}.
\end{enumerate}

\subsubsection{Optimization Conflict and Representation Failure}
The symmetric KL divergence constrains the intermediate vision and text Gaussian posteriors toward distributional agreement in both their means and variances. However, because this constraint is applied to intermediate representations while the final retrieval objective operates on normalized embeddings, the two objectives impose competing geometric constraints on the representation space.

In particular, contrastive learning requires preserving fine-grained cross-modal discrepancies to discriminate among 40 in-batch candidates, whereas the intermediate Gaussian distribution-matching penalty penalizes discrepancy between paired representations regardless of the global contrastive configuration. Under the evaluated formulation and hyperparameters ($\lambda_{\mathrm{KL}} = 10^{-3}$, clamp range $[-6.0, 2.0]$), the resulting optimization dynamics coincide with severe retrieval degradation.

While these empirical results---including near-zero Recall@1 ($1.0\%-1.8\%$) and drastic median rank inflation ($61-103$)---are consistent with a collapse-like representation failure under the evaluated formulation, the present experiments establish this empirical association but do not by themselves prove that symmetric KL uniquely causes dimensional collapse. Conclusive verification of dimensional collapse would require empirical tracking of the singular value spectrum and effective embedding rank during training, which we identify as an important direction for future investigation.
...
\begin{enumerate}
    ...
    \item \textbf{Exercise Extreme Caution with Intermediate Variational Alignment}: Do not superimpose cross-modal variational KL divergence objectives onto hyperspherical InfoNCE contrastive heads. The conflicting gradient dynamics can promote dimensional collapse under the evaluated formulation.
...
\begin{itemize}
    ...
    \item \textbf{Mechanistic Analysis Scope}: Our mechanistic interpretation identifies competing geometric pressures between intermediate distribution matching and hyperspherical contrastive repulsion, which is consistent with the observed retrieval failure; however, direct covariance-spectrum diagnostics (such as singular-value decay and effective rank tracking) were not measured and are required to establish dimensional collapse conclusively.
\end{itemize}
...
This paper presented an audited, multi-seed empirical investigation of continuous-state and variational inductive biases in Native Mamba-2 cross-modal alignment. Through a controlled 8-configuration ablation benchmark across 24 training runs on Flickr8k ($n=3$ paired runs per configuration), our findings establish that Native Mamba-2 combined with the Hamiltonian dissipative adapter (HEDO) preserves baseline-level retrieval performance under the evaluated Flickr8k, frozen-backbone, three-seed setting ($37.31 \pm 0.68\%$ vs. $37.16 \pm 0.57\%$ Mean Recall, paired $\Delta = +0.15$ pp, 95\% CI $[-1.42, +1.73]$ pp, no statistically significant difference detected), while maintaining high throughput ($7,054$ pairs/s) and low memory ($1,086$ MB peak VRAM). In contrast, Multi-Scale State Aggregation (MSA) and Adaptive Variational State Coupling (AVSC) configurations exhibit statistically significant degradation ($\Delta \approx -10.5$ pp and $\Delta \approx -31$ pp, respectively). For MSA, this degradation is consistent with recurrent positional asymmetry corrupting isotropic pooling. For AVSC, the empirical results are consistent with a representation-collapse mechanism arising from competing geometric constraints between intermediate distribution matching and hyperspherical contrastive repulsion, although direct covariance-spectrum diagnostics are required to establish dimensional collapse conclusively. These findings provide foundational empirical clarity and actionable architectural guidelines for integrating state space duality into multimodal foundation models.

% =========================================================================
% ACKNOWLEDGMENTS & REFERENCES
% =========================================================================
\section*{Acknowledgment}
The authors thank the open-source community for maintaining the \texttt{mamba\_ssm} and \texttt{transformers} libraries.
