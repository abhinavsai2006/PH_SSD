...
\caption{Efficiency vs. accuracy tradeoffs across all 8 configurations. (a) Head throughput (pairs/s) vs. Mean Recall (\%). (b) End-to-end throughput (pairs/s) vs. Peak Training VRAM (MB). Bubble area is proportional to the trainable parameter count ($465\mathrm{k}$ to $764\mathrm{k}$). Mamba-2 + HEDO lies on the observed accuracy-throughput Pareto frontier, retaining baseline-level retrieval accuracy at high throughput and low memory.}
\label{fig:efficiency_tradeoff}
\end{figure*}

Mamba-2 + HEDO requires $597,787$ trainable parameters (an increment of $132,610$ parameters over the baseline), representing a $0.06\%$ addition to the $211.2$M total model parameters. Relative to the baseline linear projection, peak training VRAM increases from $1,042.31$ MB to $1,086.15$ MB ($+43.84$ MB), head latency increases from $4.19 \pm 0.13$ ms to $5.67 \pm 0.13$ ms, and end-to-end latency increases from $7.64 \pm 0.23$ ms to $8.39 \pm 0.08$ ms. Despite these measurable overheads, head throughput remains high at $7,054.12 \pm 161.35$ pairs/s ($4,765.93 \pm 45.65$ pairs/s end-to-end), supporting Hypothesis 4 ($H_4$) by confirming practical computational throughput. As shown in Fig.~\ref{fig:efficiency_tradeoff}, Mamba-2 + HEDO lies on the observed accuracy-throughput Pareto frontier.
...
\begin{theorem}[Optimization Conflict between InfoNCE and Symmetric Variational KL]
\label{thm:gradient_conflict}
Let $\mathbf{z}_v, \mathbf{z}_t \in \mathbb{S}^{d-1}$ be $\ell_2$-normalized embeddings on the unit hypersphere supervised by multi-positive InfoNCE loss $\mathcal{L}_{\mathrm{NCE}}$ and symmetric variational coupling loss $\mathcal{L}_{\mathrm{KL}}$. While InfoNCE promotes discriminative separation of normalized cross-modal embeddings across $\mathbb{S}^{d-1}$, the symmetric KL term penalizes discrepancies between the intermediate vision and text posterior means and variances:
\begin{equation}
...
\end{equation}
Under the evaluated formulation, this distribution-matching objective introduces optimization pressure that can conflict with the hyperspherical geometry promoted by multi-positive InfoNCE, providing a mechanistic explanation consistent with the observed severe retrieval degradation.
\end{theorem}
